...
from .models import ActionTaken, Report, ReportStatus, Review, UserModerationProfile
...
from .services import (
    recalculate_traveler_rating,
    update_traveler_rating,
)
...
class ReviewListCreateAPIView(generics.ListCreateAPIView):
    ...
    * Senders can see reviews they have submitted.
    * Travelers can see reviews submitted about them.
    """
    ...
    def get_queryset(self):
        """
        Optimized queryset returning reviews where the user is either sender or traveler.
        """
    ...
    def perform_create(self, serializer):
        """
        Save the review, recalculate traveler rating, and trigger
        a notification to the traveler after transaction commit.
        """
        review = serializer.save(sender=self.request.user)

        # Recalculate average traveler rating
        ...
        # Notify the traveler about the new review
        transaction.on_commit(
...
class ReviewRetrieveUpdateDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    * Both sender and traveler can view (retrieve) it.
    ...
    def get_queryset(self):
        """
        Optimizes database retrieval with select_related.
        """
        user = self.request.user
        return Review.objects.filter(
            Q(sender=user) | Q(traveler=user)
    ...
    def perform_update(self, serializer):
        ...
        # Recalculate accurately when a score is updated
        transaction.on_commit(
            lambda: recalculate_traveler_rating(
                traveler=updated_review.traveler,
            )
        )

        transaction.on_commit(
            lambda: notify_review_updated(
                user=updated_review.traveler,
    ...
    def perform_destroy(self, instance):
        ...
        # Recalculate accurately after deletion
        transaction.on_commit(
            lambda: recalculate_traveler_rating(
                traveler=traveler,
            )
